...
// WithSizeInBytes add sizeInBytes to the builder
...
func (app *builder) Now() (Chunk, error) {
	...
	if app.data == nil {
		return nil, errors.New("the data hash is mandatory in order to build a Chunk instannce")
	}

	hsh, err := app.hashAdapter.FromMultiBytes([][]byte{
		[]byte(strconv.Itoa(int(app.sizeInBytes))),
		app.data.Bytes(),
	})
